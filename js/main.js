...
let pickupPlayerIndicator = null; // THREE.Group for local player foot ring
...
// Park spawn points — outside the court fence, along paths and open areas
const PICKUP_SPAWN_POINTS = [
    { x: -10, z: -24 },   // behind -Z gate, left side
    { x:  10, z: -24 },   // behind -Z gate, right side
    { x: -10, z:  24 },   // behind +Z gate, left side
    { x:  10, z:  24 },   // behind +Z gate, right side
    { x: -16, z:  -8 },   // left sideline, near bench
    { x:  16, z:  -8 },   // right sideline, near bench
    { x: -16, z:   8 },   // left sideline, far end
    { x:  16, z:   8 },   // right sideline, far end
    { x:   0, z: -28 },   // on path behind -Z gate
    { x:   0, z:  28 },   // on path behind +Z gate
];

// ─── Spectator Feed (watch active court game from the world) ──
// When a pickup game is running, spectators see the 6 game
// players on the court + the basketball as live entities.
const pickupGameEntities = []; // index 0-5 → { playerData, nametag, targetX, targetZ, targetAngle } | null
let pickupGameBall = null;     // { mesh } — spectator ball visual (no physics)
let pickupGameScores = null;   // [home, away] or null
let pickupGameActive = false;
const BALL_RADIUS = 0.1193;    // match ball.js
...
const SOLO_TIPOFF_LAYOUT = {
    // NBA-style tip-off: both jumpers face each other at dead center
    player: { x: 0, z: 0.9, facing: Math.PI },
    contestOpponent: { x: 0, z: -0.9, facing: 0 },
    // Referee stands to the side of center, facing the jumpers
    referee: { x: 1.5, z: 0, facing: Math.PI * 1.5 },
    // After tip-off, ref walks to sideline between benches (not behind them)
    refereeExit: { x: -8.8, z: -3.0, facing: Math.PI * 0.5 },
    teammates: [
...
const OOB_HALF_WIDTH = 7.62;           // court X extents (sideline)
const OOB_HALF_LENGTH = 14.325;        // court Z extents (baseline)
const OOB_FT_EXTENDED_Z = 8.535;       // free-throw line extended (HALF_LENGTH - KEY_LENGTH)
const OOB_MARGIN = 0.15;               // small margin outside lines before triggering OOB
const OOB_SIDELINE_INSET = 0.5;        // how far inside the sideline the inbounder stands
const OOB_REF_SPEED = 2.2;             // ref walk speed toward ball
const OOB_INBOUNDER_SPEED = 2.8;       // inbounder walk speed to throw-in spot
const OOB_REF_PICKUP_DIST = 0.6;       // ref picks up ball within this distance
const OOB_HANDOFF_DIST = 1.2;          // ref hands off ball within this distance
const OOB_PASS_TIMEOUT = 6.0;          // seconds to pass in before auto-pass
const OOB_SETTLE_DELAY = 0.4;          // brief pause after handoff before pass is required
let inboundState = null;               // null when not in OOB sequence
...
const cameraInput = { pitchUp: false, pitchDown: false, rotateLeft: false, rotateRight: false };
...
function resetCameraInput() { cameraInput.pitchUp = cameraInput.pitchDown = cameraInput.rotateLeft = cameraInput.rotateRight = false; }
...
const _idleInput = { forward: false, backward: false, left: false, right: false, jump: false };
const _remoteSeatedCarry = { holding: false, shooting: false, dribbling: false, dribblePhase: 0, dunking: false, hanging: false, seated: true, seatSettled: true, blocking: false };
...
// Persistent player-mode camera orbit (prevents drift)
let _playerCamTheta = 0;       // azimuthal angle (horizontal orbit)
let _playerCamPhi = 0.72;      // polar angle (pitch from Y axis)
let _camThetaVel = 0;          // smoothed angular velocity (horizontal)
let _camPhiVel = 0;            // smoothed angular velocity (vertical)
...
function setGameplayHudVisible(visible) {
    ...
    if (isSolo) {
        ...
        scoreHud?.classList.add('hud-hidden');
        oppScoreHud?.classList.add('hud-hidden');
        staminaHud?.classList.add('hud-hidden');
        controlsHint?.classList[method]('hud-hidden');
        if (visible) uiOverlay?.classList.add('title-under-sb');
        else uiOverlay?.classList.remove('title-under-sb');
        if (soloScoreboard) {
...
function positionBallForTipOffHold() {
    ...
    // Hold ball at court center between the two jumpers (ref reaches toward center)
    basketballData.mesh.position.set(0, refGroundY + 1.26, 0);
...
function beginTipOffToss() {
    ...
    basketballData.grounded = false;
    basketballData.idleFrames = 0;
    // Toss ball from dead center between the two jumpers
    basketballData.mesh.position.set(0, refGroundY + 1.55, 0);
    basketballData.prevPosition.copy(basketballData.mesh.position);
...
// ─── Out-of-Bounds / Inbounding System ─────────────────────────
// Phases: 'detect' → 'ref_retrieve' → 'ref_deliver' → 'inbounder_ready' → 'passing' → done

/** Check if ball position is outside the court boundary lines. */
function isBallOutOfBounds(ball) {
    if (!ball?.mesh?.visible || !ball.active) return false;
    if (ball.heldByPlayer) return false;
    const p = ball.mesh.position;
    const mx = OOB_HALF_WIDTH + OOB_MARGIN;
    const mz = OOB_HALF_LENGTH + OOB_MARGIN;
    return Math.abs(p.x) > mx || Math.abs(p.z) > mz;
}

/**
 * Compute the throw-in spot on the boundary nearest to where the ball went out.
 * NBA rule: no nearer to the baseline than the free-throw line extended.
 */
function computeInboundSpot(ballPos) {
    const bx = ballPos.x, bz = ballPos.z;
    // Determine which boundary the ball crossed
    const overX = Math.abs(bx) - OOB_HALF_WIDTH;
    const overZ = Math.abs(bz) - OOB_HALF_LENGTH;

    let spotX, spotZ, facingAngle;

    if (overZ >= overX) {
        // Went out over a baseline (Z boundary)
        const side = Math.sign(bz);
        spotZ = side * OOB_HALF_LENGTH;
        // Clamp X to within court width
        spotX = Math.max(-OOB_HALF_WIDTH + 1.0, Math.min(OOB_HALF_WIDTH - 1.0, bx));
        // NBA rule: no closer to baseline than free-throw line extended
        const ftExtended = OOB_FT_EXTENDED_Z;
        if (Math.abs(spotZ) > ftExtended) {
            // Ball went out behind baseline — move spot to FT line extended
            spotZ = side * ftExtended;
        }
        // Inbounder faces toward court center from baseline
        facingAngle = side > 0 ? Math.PI : 0;
    } else {
        // Went out over a sideline (X boundary)
        const side = Math.sign(bx);
        spotX = side * (OOB_HALF_WIDTH - OOB_SIDELINE_INSET);
        // Clamp Z within court length
        spotZ = Math.max(-OOB_HALF_LENGTH + 1.0, Math.min(OOB_HALF_LENGTH - 1.0, bz));
        // Inbounder faces inward
        facingAngle = side > 0 ? Math.PI * 1.5 : Math.PI * 0.5;
    }

    return { x: spotX, z: spotZ, facing: facingAngle };
}

/**
 * Determine which team gets the inbound based on last-touch rule.
 * Returns 'player' (user's team) or 'opponent'.
 * The team that did NOT last touch the ball gets possession.
 */
function determineInboundTeam(ball) {
    const lastTouch = ball._lastTouchRef;
    if (!lastTouch) return 'player'; // default to player if unknown

    // If last touch was by an opponent, player team gets it
    if (opponents.includes(lastTouch)) return 'player';
    // If last touch was by player or teammate, opponents get it
    if (lastTouch === playerData || teammates.includes(lastTouch)) return 'opponent';

    return 'player'; // fallback
}

/**
 * Pick the best inbounder from the receiving team.
 * Chooses the player closest to the throw-in spot.
 */
function pickInbounder(team, spot) {
    let candidates;
    if (team === 'player') {
        candidates = [playerData, ...teammates].filter(p => p?.group?.visible && p.stunTimer <= 0);
    } else {
        candidates = opponents.filter(p => p?.group?.visible && p.stunTimer <= 0);
    }
    if (candidates.length === 0) return null;

    let best = candidates[0], bestDist = Infinity;
    for (const c of candidates) {
        const dx = c.group.position.x - spot.x;
        const dz = c.group.position.z - spot.z;
        const d = dx * dx + dz * dz;
...
/**
 * Pick the best pass target for the inbounder from the same team.
 * Chooses the teammate closest to the inbounder (but not the inbounder itself).
 */
function pickInboundTarget(team, inbounder) {
    let candidates;
    if (team === 'player') {
        candidates = [playerData, ...teammates].filter(p => p?.group?.visible && p !== inbounder && p.stunTimer <= 0);
    } else {
        candidates = opponents.filter(p => p?.group?.visible && p !== inbounder && p.stunTimer <= 0);
    }
    if (candidates.length === 0) return null;

    // Pick the closest teammate that is on-court
    let best = null, bestDist = Infinity;
    for (const c of candidates) {
        const dx = c.group.position.x - inbounder.group.position.x;
        const dz = c.group.position.z - inbounder.group.position.z;
        const d = dx * dx + dz * dz;
        if (d < bestDist) { bestDist = d; best = c; }
    }
    return best;
...
/** Initiate the OOB → inbound sequence. */
function triggerOutOfBounds() {
    if (!basketballData || !matchLive || inboundState) return;
    if (gameMode !== 'solo') return; // only in solo mode for now

    const ballPos = basketballData.mesh.position.clone();
    const team = determineInboundTeam(basketballData);
    const spot = computeInboundSpot(ballPos);

    // Freeze the ball
    basketballData.velocity.set(0, 0, 0);
    basketballData.sleeping = true;

    // Cancel any active player stances
    ...
    inboundState = {
        ...
        spot,               // { x, z, facing } throw-in location
        ballOobPos: ballPos,
        inbounder: null,    // set after ref delivers
        passTarget: null,
        passTimer: 0,
        settleTimer: 0
    };

    // Ref walks to the ball
    ref._sidelineState = null; // clear any existing sideline walk
    ref.group.visible = true;

    showShotFeedback('Out of Bounds', team === 'player' ? 'Your ball' : 'Opponent ball');
}

/** Walk an entity toward a target point. Returns true when arrived. */
function walkEntityToward(entity, tx, tz, speed, delta) {
    ...
    // Face the walk direction
    const walkAngle = Math.atan2(dx, dz);
    let angleDiff = walkAngle - (entity.facingAngle || 0);
    while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
    while (angleDiff < -Math.PI) angleDiff += Math.PI * 2;
    entity.facingAngle += angleDiff * (1 - Math.exp(-10 * delta));
    entity.group.rotation.y = entity.facingAngle;

    // Set movement input
    const input = { forward: false, backward: false, left: false, right: false, jump: false };
    if (dz < -0.1) input.forward = true;
    if (dz > 0.1) input.backward = true;
    if (dx < -0.1) input.left = true;
    if (dx > 0.1) input.right = true;

    entity.speedMultiplier = speed / 4.5; // relative to base WALK_SPEED
    updatePlayer(entity, delta, input, null, [], null);
    return false;
}

/** Stand an entity still and face a given angle. */
function standEntityIdle(entity, facingAngle, delta) {
    let angleDiff = facingAngle - (entity.facingAngle || 0);
    while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
    while (angleDiff < -Math.PI) angleDiff += Math.PI * 2;
    entity.facingAngle += angleDiff * (1 - Math.exp(-10 * delta));
    entity.group.rotation.y = entity.facingAngle;
...
/** Core inbound state machine — called each frame when inboundState is active. */
function updateInboundState(delta) {
    if (!inboundState || !basketballData || !refereeData?.group) return;

    const st = inboundState;
    const ball = basketballData;
    const ref = refereeData;
    const ballPos = ball.mesh.position;

    // ── Phase: ref_retrieve — referee walks to ball and picks it up
    if (st.phase === 'ref_retrieve') {
        const arrived = walkEntityToward(ref, ballPos.x, ballPos.z, OOB_REF_SPEED, delta);

        // Keep ball visible on ground while ref approaches
        ball.sleeping = true;
        ball.velocity.set(0, 0, 0);

        if (arrived || distXZ(ref.group.position, ballPos) < OOB_REF_PICKUP_DIST) {
            // Ref "picks up" the ball — hold it at chest
            ball.active = false;
            ball.sleeping = false;
            st.phase = 'ref_deliver';
            // Pick inbounder now
            st.inbounder = pickInbounder(st.team, st.spot);
            if (!st.inbounder) {
                // No valid inbounder — just drop ball at spot and resume
                cancelInbound();
                return;
            }
        }
    ...
    // ── Phase: ref_deliver — referee carries ball to the throw-in spot area
    if (st.phase === 'ref_deliver') {
        // Position ball at ref's hand level
        const refPos = ref.group.position;
        const refGroundY = refPos.y + (ref.visualGroundOffsetY || 0.265);
        ballPos.set(refPos.x, refGroundY + 1.26, refPos.z);
        ball.mesh.visible = true;

        // Walk the inbounder toward the throw-in spot simultaneously
        if (st.inbounder && st.inbounder !== playerData) {
            walkEntityToward(st.inbounder, st.spot.x, st.spot.z, OOB_INBOUNDER_SPEED, delta);
        } else if (st.inbounder === playerData) {
            // Auto-walk user player to the spot (override input)
            walkEntityToward(playerData, st.spot.x, st.spot.z, OOB_INBOUNDER_SPEED, delta);
        }

        // Ref walks toward a point near the throw-in spot (offset slightly)
        const refTargetX = st.spot.x + (st.spot.facing === Math.PI * 0.5 ? -1.2 : st.spot.facing === Math.PI * 1.5 ? 1.2 : 0);
        const refTargetZ = st.spot.z + (st.spot.facing === 0 ? -1.2 : st.spot.facing === Math.PI ? 1.2 : 0);
        const refArrived = walkEntityToward(ref, refTargetX, refTargetZ, OOB_REF_SPEED, delta);

        // Check if inbounder is at the spot and ref is nearby
        const inbounderAtSpot = distXZ(st.inbounder.group.position, { x: st.spot.x, z: st.spot.z }) < 1.0;

        if (refArrived && inbounderAtSpot) {
            st.phase = 'handoff';
        }
        return;
    }

    // ── Phase: handoff — ref hands ball to the inbounder
    if (st.phase === 'handoff') {
        // Snap ball to inbounder's hands
        const inbPos = st.inbounder.group.position;
        const inbGroundY = inbPos.y + (st.inbounder.visualGroundOffsetY || 0.265);
        ballPos.set(inbPos.x, inbGroundY + 1.18, inbPos.z);

        // Face inbounder toward court
        standEntityIdle(st.inbounder, st.spot.facing, delta);
        // Ref stands and faces the inbounder
        const refFace = Math.atan2(inbPos.x - ref.group.position.x, inbPos.z - ref.group.position.z);
        standEntityIdle(ref, refFace, delta);

        ball.active = false;
        ball.mesh.visible = true;

        st.settleTimer += delta;
        if (st.settleTimer >= OOB_SETTLE_DELAY) {
            st.phase = 'passing';
            st.passTimer = 0;
            // Pick best pass target
            st.passTarget = pickInboundTarget(st.team, st.inbounder);
        }
        return;
    }

    // ── Phase: passing — inbounder must pass the ball in
    if (st.phase === 'passing') {
        const inbPos = st.inbounder.group.position;
        const inbGroundY = inbPos.y + (st.inbounder.visualGroundOffsetY || 0.265);
        ballPos.set(inbPos.x, inbGroundY + 1.18, inbPos.z);
        ...
        // Inbounder holds position, faces court
        standEntityIdle(st.inbounder, st.spot.facing, delta);
        // Ref idles nearby
        const refFace = Math.atan2(inbPos.x - ref.group.position.x, inbPos.z - ref.group.position.z);
        standEntityIdle(ref, refFace, delta);
        ...
        // AI inbounder auto-passes after a brief delay
        if (st.inbounder !== playerData) {
            if (st.passTimer >= 1.0 && st.passTarget) {
                executeInboundPass(st.inbounder, st.passTarget);
                return;
            }
            // No target — just drop ball and resume after timeout
            if (st.passTimer >= OOB_PASS_TIMEOUT) {
                cancelInbound();
                return;
            }
        } else {
            // Player is the inbounder — they press Z to pass in
            // (handled in the input section — passQueued triggers executeInboundPass)
            // Auto-pass on timeout
            if (st.passTimer >= OOB_PASS_TIMEOUT && st.passTarget) {
                executeInboundPass(playerData, st.passTarget);
                return;
            }
        }
        return;
...
/** Execute the inbound pass and end the inbound sequence. */
function executeInboundPass(from, target) {
    if (!basketballData || !from || !target) {
        cancelInbound();
        return;
    }
    ...
    // Activate ball and give it to the inbounder momentarily
    ball.active = true;
    ball.sleeping = false;
    ball.heldByPlayer = true;
    ball.heldByPlayerData = from;
    ball._lastTouchRef = from;

    // Fire pass
    const targetPos = new THREE.Vector3(
        target.group.position.x,
        target.group.position.y + (target.visualGroundOffsetY || 0.265) + 1.18,
        target.group.position.z
    );
    passBallToTarget(ball, from, targetPos, 'chest');

    // End inbound
    finalizeInbound();
}

/** Clean up inbound state and resume play. */
function finalizeInbound() {
    inboundState = null;
    matchLive = true;

    // Restore ref speed and send to sideline
    if (refereeData) {
        refereeData.speedMultiplier = REF_BASE_SPEED / 4.5;
        refereeData._sidelineState = {
            phase: 'walking',
            target: { ...SOLO_TIPOFF_LAYOUT.refereeExit },
            idleFacing: SOLO_TIPOFF_LAYOUT.refereeExit.facing
...
/** Cancel inbound (no valid target) — drop ball at spot and resume. */
function cancelInbound() {
    if (!inboundState || !basketballData) {
        inboundState = null;
        matchLive = true;
        return;
    }
    const spot = inboundState.spot;
    basketballData.mesh.position.set(spot.x, 1.5, spot.z);
    basketballData.velocity.set(0, 0, 0);
    basketballData.active = true;
    basketballData.sleeping = false;
    basketballData.heldByPlayer = false;
...
/** Utility: XZ distance between two positions. */
function distXZ(a, b) {
    const dx = a.x - (b.x ?? 0);
    const dz = a.z - (b.z ?? 0);
...
/** Check if inbound is currently active. */
function isInboundActive() {
    return !!inboundState;
}
...
function enterPickupWorld() {
    ...
    const spawn = PICKUP_SPAWN_POINTS[Math.floor(Math.random() * PICKUP_SPAWN_POINTS.length)];
    playerData.group.position.set(spawn.x, groundedY, spawn.z);
    // Face toward court center
    playerData.facingAngle = Math.atan2(-spawn.x, -spawn.z);
    ...
    // Set jersey to neutral gray (not queued yet)
    setLocalJerseyColor(0x777777);

    // Create queue zones if not done yet
    if (!pickupZonesCreated) createPickupQueueZones();
    setPickupZonesVisible(true);

    // Create player foot indicator if needed
    if (!pickupPlayerIndicator) pickupPlayerIndicator = createPickupPlayerIndicator();

    // Start network sync
    startPickupSync({
        mySessionId: pickupWorldSessionId,
        onWorldState: handlePickupWorldState,
        onGameState: handlePickupGameState
    });
    ...
    const exitBtn = document.querySelector('.pickup-exit-btn');
...
function cleanupPickupWorld() {
    ...
    // Restore jersey to default red
    setLocalJerseyColor(TEAMMATE_JERSEY_COLOR);
    ...
    // Clean up spectator game entities
    cleanupPickupGameEntities();
...
// ─── Player Foot Indicator (pickup world only, local player) ──

function createPickupPlayerIndicator() {
    const group = new THREE.Group();

    // Outer ring — subtle warm glow
    ...
    const outerRing = new THREE.Mesh(
        new THREE.RingGeometry(0.42, 0.50, 48),
        outerMat
    );
    outerRing.rotation.x = -Math.PI / 2;
    outerRing.renderOrder = 996;
    group.add(outerRing);

    // Inner ring — brighter accent
    ...
    const innerRing = new THREE.Mesh(
        new THREE.RingGeometry(0.34, 0.41, 48),
        innerMat
    );
    innerRing.rotation.x = -Math.PI / 2;
    innerRing.renderOrder = 996;
    group.add(innerRing);
...
function updatePickupPlayerIndicator(delta) {
    if (!pickupPlayerIndicator) return;
    if (!pickupWorldActive || !playerData?.group?.visible) {
        pickupPlayerIndicator.visible = false;
        return;
    }

    pickupPlayerIndicator.visible = true;
    const pp = playerData.group.position;
    pickupPlayerIndicator.position.set(pp.x, 0.03, pp.z);

    // Gentle breathing pulse
    const t = performance.now() * 0.001;
    const pulse = 0.3 + Math.sin(t * 1.8) * 0.08;
    pickupPlayerIndicator.userData.outerMat.opacity = pulse;
    pickupPlayerIndicator.userData.innerMat.opacity = pulse * 0.6;
}

// ─── Pickup World Chat ─────────────────────────────────────

let pickupChatInitialized = false;
const chatNameColors = new Map();

// Warm, readable name color palette — high saturation, good contrast on dark
const CHAT_NAME_PALETTE = [
    '#ffb074', '#74d4ff', '#a8e66a', '#ff7eb3', '#ffda5c',
    '#7ee8d0', '#c49bff', '#ff9d6e', '#69d2e7', '#e8c14a',
    '#8fd9a8', '#f2808a', '#93c8ff', '#d4a5ff', '#ffcc7a',
    '#6bcfb8', '#ff8fa0', '#a5d6f7', '#c6e870', '#f5b9d4'
];

function getChatNameColor(name) {
    if (chatNameColors.has(name)) return chatNameColors.get(name);
    let hash = 0;
    for (let i = 0; i < name.length; i++) {
        hash = ((hash << 5) - hash + name.charCodeAt(i)) | 0;
    }
    const color = CHAT_NAME_PALETTE[Math.abs(hash) % CHAT_NAME_PALETTE.length];
    chatNameColors.set(name, color);
    return color;
}
...
function initPickupChat() {
    if (pickupChatInitialized) return;
    pickupChatInitialized = true;

    const chatEl = document.getElementById('pickup-chat');
    const toggleBtn = document.getElementById('pickup-chat-toggle');
    const badge = toggleBtn?.querySelector('.chat-badge');
    const input = document.getElementById('pickup-chat-input');
    const sendBtn = document.getElementById('pickup-chat-send');
    const log = document.getElementById('pickup-chat-log');
    ...
    function sendChat() {
        ...
        connection.send({ type: PICKUP_CHAT, text: text.slice(0, 200) });
        input.value = '';
    }
    ...
    connection.on(PICKUP_CHAT, (msg) => {
        if (!log) return;
        const from = msg.from || '?';
        const nameColor = getChatNameColor(from);
        const msgEl = document.createElement('div');
        msgEl.className = 'pickup-chat-msg';
        msgEl.innerHTML = `<span class="chat-name" style="color:${nameColor}">${escapeHtml(from)}</span>${escapeHtml(msg.text || '')}`;
        ...
        // Flash badge if chat is collapsed
        if (!chatEl.classList.contains('open') && badge) {
            badge.classList.add('active');
        }
    });
}

function showPickupChat() {
    initPickupChat();
    const chatEl = document.getElementById('pickup-chat');
    if (chatEl) chatEl.style.display = 'flex';
}

function hidePickupChat() {
    const chatEl = document.getElementById('pickup-chat');
    if (chatEl) {
...
// ─── Local Player Jersey Color (pickup world) ──────────────

function setLocalJerseyColor(hexColor) {
    if (!playerData) return;
    const c = new THREE.Color(hexColor);
    if (playerData._jerseyMat) playerData._jerseyMat.color.copy(c);
    if (playerData._headbandMat) playerData._headbandMat.color.copy(c);
    playerData.jerseyColor = hexColor;

    // Also tint the foot indicator to match team color
    if (pickupPlayerIndicator) {
        // Brighten the team color for the additive indicator
        const indicatorC = new THREE.Color(hexColor).lerp(new THREE.Color(0xffffff), 0.4);
        if (pickupPlayerIndicator.userData.outerMat) pickupPlayerIndicator.userData.outerMat.color.copy(indicatorC);
        if (pickupPlayerIndicator.userData.innerMat) pickupPlayerIndicator.userData.innerMat.color.copy(indicatorC);
    }
}
...
let _pickupPromptState = null; // track last state to avoid redundant DOM writes
...
function checkPickupZoneProximity() {
    ...
    const dhSq = (px - PICKUP_ZONE_HOME_POS.x) ** 2 + (pz - PICKUP_ZONE_HOME_POS.z) ** 2;
    const daSq = (px - PICKUP_ZONE_AWAY_POS.x) ** 2 + (pz - PICKUP_ZONE_AWAY_POS.z) ** 2;
    const rSq = PICKUP_ZONE_RADIUS * PICKUP_ZONE_RADIUS;

    const inHome = dhSq < rSq;
    const inAway = daSq < rSq;

    if (inHome && pickupMyZone !== 'home') {
        pickupMyZone = 'home';
        pickupEnterZone('home');
        setLocalJerseyColor(TEAMMATE_JERSEY_COLOR); // red
    } else if (inAway && pickupMyZone !== 'away') {
        pickupMyZone = 'away';
        pickupEnterZone('away');
        setLocalJerseyColor(OPPONENT_JERSEY_COLOR); // blue
    } else if (!inHome && !inAway && pickupMyZone !== null) {
        pickupMyZone = null;
        pickupLeaveZone();
        setLocalJerseyColor(0x777777); // neutral gray
    }

    // Update prompt text — only when state changes
    const nearZone = dhSq < 64 || daSq < 64; // 8m radius
    const newState = pickupMyZone || (nearZone ? 'near' : 'far');
    if (newState !== _pickupPromptState) {
        _pickupPromptState = newState;
        if (!_pickupPromptEl) _pickupPromptEl = document.getElementById('pickup-prompt');
        if (_pickupPromptEl) {
            if (pickupMyZone === 'home') {
                _pickupPromptEl.textContent = 'You\'re queued for HOME (red)';
                _pickupPromptEl.style.color = '#ff6b4a';
                _pickupPromptEl.style.borderColor = 'rgba(255, 80, 60, 0.3)';
                _pickupPromptEl.style.background = 'rgba(40, 10, 10, 0.65)';
            } else if (pickupMyZone === 'away') {
                _pickupPromptEl.textContent = 'You\'re queued for AWAY (blue)';
                _pickupPromptEl.style.color = '#5599ff';
                _pickupPromptEl.style.borderColor = 'rgba(80, 140, 255, 0.3)';
                _pickupPromptEl.style.background = 'rgba(10, 15, 40, 0.65)';
            } else if (nearZone) {
                _pickupPromptEl.textContent = 'Walk into the glowing zone to join a team';
                _pickupPromptEl.style.color = 'rgba(255,255,255,0.7)';
                _pickupPromptEl.style.borderColor = 'rgba(255, 255, 255, 0.1)';
                _pickupPromptEl.style.background = 'rgba(10, 12, 20, 0.6)';
            } else {
                _pickupPromptEl.textContent = 'Walk to a court entrance to join a team';
                _pickupPromptEl.style.color = 'rgba(255,220,180,0.92)';
                _pickupPromptEl.style.borderColor = 'rgba(255, 255, 255, 0.08)';
                _pickupPromptEl.style.background = 'rgba(10, 12, 20, 0.6)';
            }
        }
    }
    ...
    for (const sp of serverPlayers) {
        ...
        if (!rp) {
            ...
            const pd = createPlayer(scene, {
                ...
                spawnPosition: { x: sp.x, y: sp.y ?? undefined, z: sp.z },
                facingAngle: sp.a,
                name: `pickup-${sp.id}`,
                visible: true
            });
            ...
            pd._networkSynced = true;
            rp = { playerData: pd, nametag, targetX: sp.x, targetZ: sp.z, targetY: sp.y ?? -0.265, targetAngle: sp.a, teamColor, seated: false };
        ...
        rp.targetX = sp.x;
        rp.targetZ = sp.z;
        rp.targetY = sp.y ?? rp.targetY;
        rp.targetAngle = sp.a;

        // Sync animation state directly from server
        rp.playerData.moveBlend = sp.mb ?? 0;
        rp.playerData.walkCycle = sp.wc ?? 0;
        rp.playerData.isGrounded = !!sp.g;
        rp.playerData.isJumping = !!sp.j;
        rp.seated = !!sp.s;
...
function updateRemotePickupPlayers(delta) {
    ...
    const yLerpRate = 1 - Math.exp(-16 * delta); // faster Y for snappy jumps
    for (const [, rp] of pickupRemotePlayers) {
        const g = rp.playerData.group;
        const pd = rp.playerData;

        // Interpolate XZ position
        g.position.x += (rp.targetX - g.position.x) * lerpRate;
        g.position.z += (rp.targetZ - g.position.z) * lerpRate;

        // Estimate vertical velocity from Y delta (for jump animation)
        const prevY = g.position.y;
        // Interpolate Y position (for jumps)
        g.position.y += (rp.targetY - g.position.y) * yLerpRate;
        // Derive velocityY so animateLimbs knows up vs down phase
        if (pd.isJumping && delta > 0) {
            pd.velocityY = (g.position.y - prevY) / delta;
        } else {
            pd.velocityY = 0;
        }

        // Smooth angle interpolation
        let angleDiff = rp.targetAngle - pd.facingAngle;
        while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
        while (angleDiff < -Math.PI) angleDiff += Math.PI * 2;
        pd.facingAngle += angleDiff * lerpRate;
        g.rotation.y = pd.facingAngle;

        // moveBlend and walkCycle are synced directly from server data
        // in handlePickupWorldState — no need to derive from position delta.
        // Just run the animation update with skipPhysics = true.
        const idleInput = _idleInput;
        updatePlayer(pd, delta, idleInput, null, null, rp.seated ? _remoteSeatedCarry : null, true);
    }
}
...
function createNametagSprite(name) {
    ...
    const displayName = name.slice(0, 16);
    ...
    const padX = 28;
    const padY = 8;
    const bgW = Math.min(480, Math.max(100, textWidth + padX * 2));
    const bgH = 44;
    const bgX = (512 - bgW) / 2;
    const bgY = (96 - bgH) / 2;

    // Subtle dark gradient pill background
    const grad = ctx.createLinearGradient(bgX, bgY, bgX, bgY + bgH);
    grad.addColorStop(0, 'rgba(18, 18, 28, 0.55)');
    ...
    // Thin top highlight for frosted feel
    ...
    // Name text — soft white with subtle shadow
    ctx.font = '600 28px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
    ...
    ctx.fillText(displayName, 256, 48);
...
// Cached pickup HUD DOM refs (populated on first use)
let _pickupSlotEls = null; // [home0, home1, home2, away0, away1, away2]
let _pickupCountdownEl = null;

function _ensurePickupHUDRefs() {
    if (_pickupSlotEls) return;
    _pickupSlotEls = [];
    for (let i = 0; i < 3; i++) _pickupSlotEls.push(document.getElementById(`pickup-home-slot-${i}`));
    for (let i = 0; i < 3; i++) _pickupSlotEls.push(document.getElementById(`pickup-away-slot-${i}`));
    _pickupCountdownEl = document.getElementById('pickup-countdown-display');
}

function updatePickupQueueHUD() {
    _ensurePickupHUDRefs();

    // Home slots
    for (let i = 0; i < 3; i++) {
        const el = _pickupSlotEls[i];
        if (el) {
    ...
    // Countdown
    if (_pickupCountdownEl) {
        if (pickupQueueState.cd > 0) {
            _pickupCountdownEl.textContent = `GAME STARTING IN ${pickupQueueState.cd}`;
            _pickupCountdownEl.style.display = 'block';
        } else {
            _pickupCountdownEl.style.display = 'none';
        }
    }
}

// ─── Spectator Game Feed (watch court game from the world) ──

function handlePickupGameState(msg) {
    // null game data = game ended
    if (!msg.gp) {
        cleanupPickupGameEntities();
        updatePickupScoreHUD();
        return;
    }

    pickupGameActive = true;
    pickupGameScores = msg.sc || [0, 0];

    // Create or update the 6 game player entities
    for (let i = 0; i < 6; i++) {
        const sp = msg.gp[i];
        if (!sp) {
            // Player not present — remove if exists
            if (pickupGameEntities[i]) {
                if (pickupGameEntities[i].playerData?.group) {
                    scene.remove(pickupGameEntities[i].playerData.group);
                }
                pickupGameEntities[i] = null;
            }
            continue;
        }

        const [x, z, angle, nickname] = sp;
        const team = i < 3 ? 'home' : 'away';
        const jerseyColor = team === 'home' ? 0xcc2222 : 0x2266cc;

        if (!pickupGameEntities[i]) {
            // Create new game player entity
            const pd = createPlayer(scene, {
                jerseyColor,
                spawnPosition: { x, y: undefined, z },
                facingAngle: angle,
                name: `court-${team}-${i % 3}`,
                visible: true
            });
            const nametag = createNametagSprite(nickname || 'Player');
            pd.group.add(nametag);

            pickupGameEntities[i] = {
                playerData: pd, nametag,
                targetX: x, targetZ: z, targetAngle: angle
            };
        } else {
            // Update interpolation targets
            pickupGameEntities[i].targetX = x;
            pickupGameEntities[i].targetZ = z;
            pickupGameEntities[i].targetAngle = angle;
        }
    }

    // Create or update spectator ball
    if (msg.b) {
        if (!pickupGameBall) {
            const geo = new THREE.SphereGeometry(BALL_RADIUS, 16, 12);
            const mat = new THREE.MeshStandardMaterial({
                color: 0xcc6622, roughness: 0.75, metalness: 0.02
            });
            const mesh = new THREE.Mesh(geo, mat);
            mesh.castShadow = true;
            scene.add(mesh);
            pickupGameBall = { mesh, targetX: msg.b[0], targetY: msg.b[1], targetZ: msg.b[2] };
        }
        pickupGameBall.targetX = msg.b[0];
        pickupGameBall.targetY = msg.b[1];
        pickupGameBall.targetZ = msg.b[2];
    }

    updatePickupScoreHUD();
}

function updatePickupGameEntities(delta) {
    if (!pickupGameActive) return;
    const lerpRate = 1 - Math.exp(-14 * delta);

    // Interpolate game player positions
    for (let i = 0; i < 6; i++) {
        const ent = pickupGameEntities[i];
        if (!ent) continue;
        const g = ent.playerData.group;

        // Position
        g.position.x += (ent.targetX - g.position.x) * lerpRate;
        g.position.z += (ent.targetZ - g.position.z) * lerpRate;

        // Angle
        let angleDiff = ent.targetAngle - ent.playerData.facingAngle;
        while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
        while (angleDiff < -Math.PI) angleDiff += Math.PI * 2;
        ent.playerData.facingAngle += angleDiff * lerpRate;
        g.rotation.y = ent.playerData.facingAngle;

        // Drive walk animation from movement speed
        const speed = Math.sqrt(
            (ent.targetX - g.position.x) ** 2 + (ent.targetZ - g.position.z) ** 2
        ) / Math.max(delta, 0.001);
        ent.playerData.moveBlend += ((speed > 0.3 ? 1 : 0) - ent.playerData.moveBlend) * (1 - Math.exp(-10 * delta));
        if (ent.playerData.moveBlend > 0.01) {
            ent.playerData.walkCycle = (ent.playerData.walkCycle || 0) + delta * 3.5;
        }

        // Animation-only player update (no input, no collisions)
        const idleInput = _idleInput;
        updatePlayer(ent.playerData, delta, idleInput, null, null, null, true);
    }

    // Interpolate ball position
    if (pickupGameBall) {
        const bm = pickupGameBall.mesh;
        bm.position.x += (pickupGameBall.targetX - bm.position.x) * lerpRate;
        bm.position.y += (pickupGameBall.targetY - bm.position.y) * lerpRate;
        bm.position.z += (pickupGameBall.targetZ - bm.position.z) * lerpRate;
    }
}

function cleanupPickupGameEntities() {
    for (let i = 0; i < pickupGameEntities.length; i++) {
        if (pickupGameEntities[i]?.playerData?.group) {
            scene.remove(pickupGameEntities[i].playerData.group);
        }
        pickupGameEntities[i] = null;
    }
    pickupGameEntities.length = 0;

    if (pickupGameBall) {
        scene.remove(pickupGameBall.mesh);
        pickupGameBall.mesh.geometry.dispose();
        pickupGameBall.mesh.material.dispose();
        pickupGameBall = null;
    }

    pickupGameActive = false;
    pickupGameScores = null;
}

function updatePickupScoreHUD() {
    const bar = document.getElementById('pickup-game-bar');
    if (!bar) return;

    if (pickupGameActive && pickupGameScores) {
        bar.style.display = '';
        const homeEl = bar.querySelector('.home-score');
        const awayEl = bar.querySelector('.away-score');
        if (homeEl) homeEl.textContent = pickupGameScores[0];
        if (awayEl) awayEl.textContent = pickupGameScores[1];
    } else {
...
function updatePickupWorld(delta) {
    ...
    setPickupPosition(
        ...
        playerData.group.position.y,
        playerData.moveBlend,
        playerData.walkCycle,
    ...
    // Interpolate spectator game entities (players on court + ball)
    updatePickupGameEntities(delta);
}
...
function updateOpponentAI(opp, delta) {
    ...
    if (oppHoldsBall) {
        ...
        // Check if pressured by enemies (player or teammates) — continuous each frame
        let nearestEnemyDist = Infinity;
        let nearbyEnemyCount = 0;
        if (playerData?.group?.visible && playerData.stunTimer <= 0) {
            const d = Math.hypot(playerData.group.position.x - oppPos.x, playerData.group.position.z - oppPos.z);
        ...
        const pressured = nearestEnemyDist < 2.8;
        const swarmed = nearbyEnemyCount >= 2;

        // Pass to open teammate when pressured/swarmed or held too long
        const passHoldThreshold = swarmed ? 0.12 : 0.3;
        if ((pressured && opp._holdTimer > passHoldThreshold) || (swarmed && opp._holdTimer > 0.08) || opp._holdTimer > 3.5) {
        ...
        const atDriveTarget = opp._driveTarget && Math.hypot(opp._driveTarget.x - oppPos.x, opp._driveTarget.z - oppPos.z) < 1.2;
        const driveStale = (opp._driveAge = (opp._driveAge || 0) + delta) > 1.8;
        if (!opp._driveTarget || opp._holdTimer < 0.05 || atDriveTarget || driveStale || (swarmed && opp._driveAge > 0.5)) {
            opp._driveAge = 0;
            // Pick a drive target that avoids defenders
            const seed = opp._holdSeed || 0.5;
            const baseX = (seed > 0.5 ? 3.5 : -3.5) * (seed - 0.25);
            const baseZ = OPP_TARGET_RIM_Z - 3.0 - Math.random() * 4.0;
            let bestX = baseX, bestZ = baseZ, bestClear = 0;
            // Try a few candidates and pick the one with most clearance from defenders
            for (let attempt = 0; attempt < 5; attempt++) {
                const cx = (attempt === 0) ? baseX : -6 + Math.random() * 12;
                const cz = (attempt === 0) ? baseZ : OPP_TARGET_RIM_Z - 2.0 - Math.random() * 6.0;
                let minD = Infinity;
                if (playerData?.group?.visible) minD = Math.min(minD, Math.hypot(playerData.group.position.x - cx, playerData.group.position.z - cz));
                for (const tm2 of teammates) {
                    if (!tm2.group.visible || tm2.stunTimer > 0) continue;
                    minD = Math.min(minD, Math.hypot(tm2.group.position.x - cx, tm2.group.position.z - cz));
                }
                if (minD > bestClear) { bestClear = minD; bestX = cx; bestZ = cz; }
            }
            opp._driveTarget = { x: bestX, z: bestZ };
        }
    ...
    if (basketballData?.heldByPlayer && basketballData.heldByPlayerData !== opp) {
        ...
        if (isTeammateOpp && holder?.group?.visible) {
            ...
            if (!opp._positionTarget || Math.random() < 0.02) {
                // Spread out near the rim: pick a spot away from other opponents
                const idx = opponents.indexOf(opp);
                const slots = [{ x: -5, z: OPP_TARGET_RIM_Z - 6 }, { x: 5, z: OPP_TARGET_RIM_Z - 6 },
                               { x: 0, z: OPP_TARGET_RIM_Z - 9 }, { x: -3, z: OPP_TARGET_RIM_Z - 3 },
                               { x: 3, z: OPP_TARGET_RIM_Z - 3 }];
...
function findOpenOpponentForPass(fromOpp) {
    let best = null;
    let bestScore = -Infinity;
    const fromPos = fromOpp.group.position;

    // Determine how pressured the passer is — lower openness threshold when swarmed
    let passerNearbyCount = 0;
    const enemies = [playerData, ...teammates];
    for (const e of enemies) {
        if (!e?.group?.visible || e.stunTimer > 0) continue;
        if (Math.hypot(e.group.position.x - fromPos.x, e.group.position.z - fromPos.z) < 3.5) passerNearbyCount++;
    }
    ...
    for (const other of opponents) {
        ...
        for (const e of enemies) {
            ...
            const d = Math.hypot(e.group.position.x - otherPos.x, e.group.position.z - otherPos.z);
            if (d < nearestEnemyDist) nearestEnemyDist = d;
        }
        if (nearestEnemyDist < minOpenness) continue;

        const openness = Math.min(nearestEnemyDist, 8);
        const rimProximity = Math.max(0, 20 - Math.hypot(otherPos.x, otherPos.z - 12.73));

        // Penalize if an enemy is standing in the pass lane
        const passDx = otherPos.x - fromPos.x;
        const passDz = otherPos.z - fromPos.z;
        ...
        if (passDist > 0.5) {
            ...
            for (const e of enemies) {
                if (!e?.group?.visible) continue;
                const ox = e.group.position.x - fromPos.x;
                const oz = e.group.position.z - fromPos.z;
                const proj = ox * laneUx + oz * laneUz;
                if (proj < 0.5 || proj > passDist - 0.5) continue;
                const perpDist = Math.abs(ox * laneUz - oz * laneUx);
                if (perpDist < 1.5) lanePenalty += (1.5 - perpDist) * 4;
            }
        }

        const score = openness * 3 + rimProximity - lanePenalty;
        if (score > bestScore) {
            bestScore = score;
            best = other;
        }
    ...
    for (const attacker of allEntities) {
        ...
        for (const target of allEntities) {
            ...
            if (basketballData?.heldByPlayer && basketballData.heldByPlayerData === target) {
                ...
                forceDropBall(basketballData, hitDirX, hitDirZ, attacker);
            }
        ...
        if (!tm._driveTarget || tm._holdTimer < 0.05 || atDriveTarget || tmDriveStale || (swarmed && tm._driveAge > 0.5)) {
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
                ...
                for (const opp2 of opponents) {
                    ...
                    minD = Math.min(minD, Math.hypot(opp2.group.position.x - cx, opp2.group.position.z - cz));
                }
                if (minD > bestClear) { bestClear = minD; bestX = cx; bestZ = cz; }
            }
            tm._driveTarget = { x: bestX, z: bestZ };
        }
...
function buildScene() {
    ...
    createCity(scene);
    const cityColliders = scene.userData.cityColliders || [];
    playerColliders = hoopColliders.concat(parkColliders).concat(cityColliders);
...
function onKeyDown(e) {
    ...
    if (cameraMode === 'freeroam') {
        ...
        if (tipOffContest) {
            switch (e.code) {
                case 'ArrowUp':    playerInput.forward = true; e.preventDefault(); break;
                case 'ArrowDown':  playerInput.backward = true; e.preventDefault(); break;
                case 'ArrowLeft':  playerInput.left = true; e.preventDefault(); break;
                case 'ArrowRight': playerInput.right = true; e.preventDefault(); break;
                case 'KeyW': cameraInput.pitchUp = true; e.preventDefault(); break;
                case 'KeyS': cameraInput.pitchDown = true; e.preventDefault(); break;
                case 'KeyA': cameraInput.rotateLeft = true; e.preventDefault(); break;
                case 'KeyD': cameraInput.rotateRight = true; e.preventDefault(); break;
        ...
        if (shootingStance || passingStance) {
            // In shooting/pass stance: arrows = aim/turn, X = fire, C = cancel
            // WASD still controls camera look
            switch (e.code) {
                case 'ArrowUp':    shootInput.aimUp = true; e.preventDefault(); break;
                case 'ArrowDown':  shootInput.aimDown = true; e.preventDefault(); break;
                case 'ArrowLeft':  shootInput.turnLeft = true; e.preventDefault(); break;
                case 'ArrowRight': shootInput.turnRight = true; e.preventDefault(); break;
                case 'KeyW': cameraInput.pitchUp = true; e.preventDefault(); break;
                case 'KeyS': cameraInput.pitchDown = true; e.preventDefault(); break;
                case 'KeyA': cameraInput.rotateLeft = true; e.preventDefault(); break;
                case 'KeyD': cameraInput.rotateRight = true; e.preventDefault(); break;
            ...
            // Normal play: arrows = movement, WASD = camera look
            switch (e.code) {
                case 'ArrowUp':    playerInput.forward = true; e.preventDefault(); break;
                case 'ArrowDown':  playerInput.backward = true; e.preventDefault(); break;
                case 'ArrowLeft':  playerInput.left = true; e.preventDefault(); break;
                ...
                case 'Space': playerInput.jump = true; e.preventDefault(); break;
                case 'KeyZ':
                    // Inbound pass-in: Z fires the inbound pass
                    if (isInboundActive() && inboundState?.phase === 'passing' && inboundState?.inbounder === playerData) {
                        passQueued = true;
                    // If holding ball and teammates exist → pass; otherwise pickup
                    } else if (basketballData?.heldByPlayer && basketballData.heldByPlayerData === playerData && teammates.length > 0) {
                        passQueued = true;
                    } else {
                        pickupQueued = true;
                        pickupAssistTimer = PICKUP_ASSIST_DURATION;
...
function onKeyUp(e) {
    ...
    switch (e.code) {
        // Arrow keys — movement + stance aim
        case 'ArrowUp':
        ...
        // WASD — camera look (in player mode) / movement (in freeroam)
        case 'KeyW':
            moveState.forward = false; cameraInput.pitchUp = false; break;
        case 'KeyS':
            moveState.backward = false; cameraInput.pitchDown = false; break;
        case 'KeyA':
            moveState.left = false; cameraInput.rotateLeft = false; break;
        case 'KeyD':
            moveState.right = false; cameraInput.rotateRight = false; break;
...
// ─── WASD Camera Look + Gentle Auto-Follow ──────────────────
// Uses persistent authoritative angles (no drift from target movement).
// OrbitControls rotation is disabled in player mode — WASD handles it.
// Scroll wheel zoom still works via OrbitControls.
const CAM_ROTATE_SPEED = 2.4;  // radians/sec for A/D
const CAM_PITCH_SPEED  = 1.6;  // radians/sec for W/S
const CAM_ACCEL_RATE   = 10;   // velocity smoothing rate

function initPlayerCameraAngles() {
    _camOffset.subVectors(camera.position, controls.target);
    _camSpherical.setFromVector3(_camOffset);
    _playerCamTheta = _camSpherical.theta;
    _playerCamPhi = _camSpherical.phi;
    _camThetaVel = 0;
    _camPhiVel = 0;
}

function applyCameraLookAndFollow(delta) {
    if (!playerData) return;

    // Read radius from current distance (scroll wheel zoom changes this)
    _camOffset.subVectors(camera.position, controls.target);
    const radius = Math.max(controls.minDistance, Math.min(controls.maxDistance, _camOffset.length()));

    // ── Velocity-smoothed WASD camera input ──────────────
    const targetThetaVel = ((cameraInput.rotateLeft ? 1 : 0) - (cameraInput.rotateRight ? 1 : 0)) * CAM_ROTATE_SPEED;
    const targetPhiVel = ((cameraInput.pitchDown ? 1 : 0) - (cameraInput.pitchUp ? 1 : 0)) * CAM_PITCH_SPEED;

    const accel = 1 - Math.exp(-CAM_ACCEL_RATE * delta);
    _camThetaVel += (targetThetaVel - _camThetaVel) * accel;
    _camPhiVel += (targetPhiVel - _camPhiVel) * accel;

    _playerCamTheta += _camThetaVel * delta;
    ...
    // ── Set camera position from persistent angles ───────
    _camSpherical.set(radius, _playerCamPhi, _playerCamTheta);
...
function switchCameraMode(mode) {
    ...
    if (mode === 'orbit') {
        ...
        if (isPointerLocked) document.exitPointerLock();
        controls.enabled = true;
        controls.enableRotate = false;  // WASD handles rotation
        controls.enablePan = false;
        if (playerData) {
            playerData.group.visible = true;
            const pp = playerData.group.position;
            const groundY = pp.y + (playerData.visualGroundOffsetY || 0);
            const isPickup = pickupWorldActive || gameMode === 'pickup-world';
            // Position camera behind and above the player
            const camDist = isPickup ? 5.5 : 8;
            const camHeight = isPickup ? 2.8 : 4;
            camera.position.set(pp.x, groundY + camHeight, pp.z + camDist);
            controls.target.set(pp.x, groundY + 1.2, pp.z);
            controls.minDistance = isPickup ? 3 : 3;
            controls.maxDistance = isPickup ? 12 : 18;
            controls.update();
            initPlayerCameraAngles(); // capture initial orbit angles
        }
        resetMoveState();
        resetCameraInput();
    }
...
function animate() {
    ...
    if (startMenuActive) {
        ...
        if (pickupWorldActive) {
            // Build minimal carryState for seated pose in pickup world
            let pickupCarryState = null;
            if (sitState) {
                _carryState.holding = false;
                _carryState.shooting = false;
                _carryState.dribbling = false;
                _carryState.dribblePhase = 0;
                _carryState.dunking = false;
                _carryState.hanging = false;
                _carryState.seated = true;
                _carryState.seatSettled = sitState.phase === 'sit';
                _carryState.blocking = false;
                pickupCarryState = _carryState;
                // Freeze player while seated
                resetPlayerInput();
                playerData.velocity.set(0, 0, 0);
                playerData.velocityY = 0;
                playerData.isGrounded = true;
            ...
            updatePlayer(playerData, delta, playerInput, playerMoveBasis, playerColliders, pickupCarryState);
            updatePickupWorld(delta);
            updatePickupPlayerIndicator(delta);
        ...
        if (!pickupWorldActive && isInboundActive() && inboundState?.phase === 'passing'
            ...
            if (target) {
                executeInboundPass(playerData, target);
            }
            passQueued = false;
            pickupQueued = false;
            shootQueued = false;
    ...
    if (skipLocalAI) {
        ...
        // During inbound: all AI players stand idle (no chasing, no pickups)
        const idleInput = _idleInput;
        for (const opp of opponents) {
    ...
    if (skipLocalAI) {
        ...
        // During inbound: teammates stand idle
        const idleInput = _idleInput;
        for (const tm of teammates) {
            if (tm === inboundState?.inbounder) continue; // inbounder handled by inbound system
